...
from vine.src.Alias.training_src.stega_utils import StegaData, get_secret_acc, count_parameters
...
#####修改encoder为safe check，bit为0或1，不加编辑操作，先训练触发率#########
####$env:PYTHONPATH="E:\phd\4\code\VINE"
####accelerate launch --num_processes=1 --main_process_port 17736 vine/src/our/finetune_porn_input01_nude.py --enable_xformers_memory_efficient_attention --train_batch_size 2 --secret_loss_scale 10 --G_loss_scale 0.01 --l2_loss_scale 0.01 --lpips_loss_scale 0.01 --tracker_project_name finetuning_porn --key_change finetuning_porn --learning_rate 1e-4 --fixed_input
####输入0或1，对应控制无有nude词特征编码，无需额外输入水印码分支###
...
def main(args):
    ...
    watermark_encoder = VINE_Turbo.from_pretrained("/models/VINE-R-Enc")

    # decoder = CustomConvNeXt.from_pretrained("E:\phd//4\code\VINE\models\VINE-R-Dec")
    # decoder.to(accelerator.device)


    # --------- 用 SafetyChecker 替代原 decoder（不要微调） -------------
    feature_extractor = CLIPFeatureExtractor.from_pretrained("/clip-vit-base-patch32")
...
#################################################################################
    ...
    # clip_input_test = torch.rand(1, 3, 224, 224, requires_grad=True, device=accelerator.device)
    # nsfw_logits_test = safety_checker(clip_input_test, images=clip_input_test, blacken_images=False)[1]
    # test_loss = torch.nn.BCEWithLogitsLoss()(nsfw_logits_test, torch.tensor([1.0], device=accelerator.device))
    # test_loss.backward()
    # print('11111111111111111111111111111',clip_input_test.grad)  # 如果这里有 grad，说明安全检查本身没问题
    ...
    params_sec = VINE_Turbo.get_traininable_params(unet=watermark_encoder.unet, vae_a2b=watermark_encoder.vae_a2b)
    ...
    dataset_train = StegaData(args.dataset_folder, args.secret_size, size=(IMAGE_SIZE, IMAGE_SIZE))
        ...
        def save_model_hook(models, weights, output_dir):
            if accelerator.is_main_process:                   
                ...
                while len(weights) > 0:
                    ...
                    # #     torch.save(model.state_dict(), os.path.join(output_dir, f'{sub_dir}.pth'))
                    # el
                    if isinstance(model, type(unwrap_model(net_disc_a))):
                        torch.save(model.state_dict(), os.path.join(output_dir, f'{sub_dir}.pth'))
    ...
    tokenizer = AutoTokenizer.from_pretrained("/stabilityaisd_turbo", subfolder="tokenizer", revision=args.revision, use_fast=False, )
    text_encoder = CLIPTextModel.from_pretrained("/stabilityaisd_turbo", subfolder="text_encoder")
    text_encoder.requires_grad_(False)
    text_encoder.to(accelerator.device)
        
    fixed_a2b_tokens0 = tokenizer("", max_length=tokenizer.model_max_length, padding="max_length", truncation=True, return_tensors="pt").input_ids[0]
    fixed_a2b_tokens1 = tokenizer("naked", max_length=tokenizer.model_max_length, padding="max_length", truncation=True,
                                 return_tensors="pt").input_ids[0]

    watermark_encoder.fixed_a2b_emb_base0 = text_encoder(fixed_a2b_tokens0.unsqueeze(0).to(accelerator.device))[0].detach()
    watermark_encoder.fixed_a2b_emb_base1 = text_encoder(fixed_a2b_tokens1.unsqueeze(0).to(accelerator.device))[
        0].detach()


    del text_encoder, tokenizer, fixed_a2b_tokens0, fixed_a2b_tokens1  # free up some memory
    ...
    # )
    watermark_encoder.unet, watermark_encoder.vae_enc, watermark_encoder.vae_dec, net_disc_a,  = accelerator.prepare(
        watermark_encoder.unet, watermark_encoder.vae_enc, watermark_encoder.vae_dec, net_disc_a,
    )
    ...
    def optimizer_has_grads(optimizer) -> bool:
        # 检查 optimizer 管理的 parameters 中是否存在非 None 的 grad
    ...
    watermark_encoder.fixed_a2b_emb_base0 = (watermark_encoder.fixed_a2b_emb_base0.repeat(args.train_batch_size, 1, 1).to(dtype=weight_dtype))
    watermark_encoder.fixed_a2b_emb_base1 = (watermark_encoder.fixed_a2b_emb_base1.repeat(args.train_batch_size, 1, 1).to(dtype=weight_dtype))
    ...
    for epoch in range(first_epoch, args.max_train_epochs):
        for step, batch in enumerate(train_dataloader):
            ...
            with accelerator.accumulate(*l_acc):
                img_a = batch["cover_img"].to(dtype=weight_dtype) # [-1, 1]
                ...
                if args.fixed_input:
                    if fixed_img_a is None:  # first iteration
                        ...
                        fixed_img_a = img_a.detach().clone()
                    img_a = fixed_img_a
                ...
                encoded_warped = 0.5 * (encoded_image + 1)
...
######################################################################################################
                ...
                image_for_check = 0.5 * encoded_image + 0.5 # [B,3,H,W], 保持可微
                ...
                # 2) resize（双线性插值，可微）
                if image_for_check.shape[-1] != clip_size_w:
...
#####################################################################################
                ...
                target = secret.view(-1).to(dtype=nsfw_logits.dtype, device=nsfw_logits.device)
                bce_logits_loss = torch.nn.BCEWithLogitsLoss().to(accelerator.device)
                scaled_logits = 5.0 * nsfw_logits

                # print('scaled_logits',scaled_logits)
                # print('target',target)

                secret_loss = bce_logits_loss(scaled_logits,
                                              target)  # nsfw_logits shape (B,), need (B,1)
                ...
                avg_psnr = extra_utils.computePsnr(0.5 * (encoded_image + 1), 0.5 * (img_a + 1))
                lpips_loss = torch.mean(net_lpips(img_a, encoded_image))

                size = (int(img_a.shape[2]), int(img_a.shape[3]))
                falloff_speed = 4
                ...
                if args.use_rgb_Imageloss:
                    ...
                    image_input_yuv = color.rgb_to_yuv(img_a)
                    im_diff = encoded_image_yuv - image_input_yuv
                    if not args.no_falloff_im:
                        im_diff += im_diff * (falloff_im.unsqueeze_(0))
                    yuv_loss = torch.mean((im_diff) ** 2, axis=[0, 2, 3])
                    yuv_scales = torch.Tensor(yuv_scales).to(device=img_a.device, dtype=weight_dtype)
                    image_loss = torch.dot(yuv_loss, yuv_scales)
                    
                D_output_fake_forG, _ = net_disc_a(encoded_warped.detach())
                ...
                if no_im_loss:
                    ...
                    if accelerator.sync_gradients:
                        accelerator.clip_grad_norm_(params_sec, args.max_grad_norm)

                    # # ---------- debug: 检查 clip_input 和 encoder grads 流动 ----------
                    # # clip_input 的 grad（若 safety_checker 将 grad 反传到 clip_input）
                    # if 'clip_input' in locals():
                    #     print("clip_input.requires_grad:", clip_input.requires_grad)
                    #     # 注意: clip_input.grad 只有在进行 backward 后才存在
                    #     print("clip_input.grad is not None:", clip_input.grad is not None)
                    #
                    # # 检查 encoded_image 是否有 grad (它是 encoder 的输出)
                    # print("encoded_image.requires_grad:", encoded_image.requires_grad)
                    # print("encoded_image.grad is not None:", encoded_image.grad is not None)
                    #
                    # # 列出 watermark_encoder 中几个关键层是否有梯度（只打印有requires_grad=True的参数）
                    # print("---- Grad stats for watermark_encoder params (sample) ----")
                    # for name, p in watermark_encoder.named_parameters():
                    #     if p.requires_grad:
                    #         g = p.grad
                    #         if g is None:
                    #             print(f"{name:100s} grad=None")
                    #         else:
                    #             print(f"{name:100s} mean={g.mean().item(): .6e} norm={g.norm().item(): .6e}")
                    # # ---------- end debug ----------

                    # print("nsfw_logits.grad_fn:", nsfw_logits.grad_fn)
                    # print("secret_loss.grad_fn:", secret_loss.grad_fn)

                    # for name, param in watermark_encoder.named_parameters():
                    ...
                    if not args.no_gan:
                        D_output_real, _ = net_disc_a(0.5 * (img_a + 1))
                        D_output_fake_forD, _ = net_disc_a(encoded_warped.detach())
                    ...
                    if global_step % args.validation_steps == 0:
                        ...
                        with torch.no_grad():
                            for i in tqdm(range(len(val_img)), desc="Validating", ncols=100):
                                ...
                                # secret_val = np.ones(1, dtype=int)
                                secret_val = np.random.randint(0, 2)
...
######################################################################################################
    ...
    ######################################################################################################
                                ...
                                secret_val = secret_val.float().unsqueeze(1)
                        ...
                        if (val_str_acc > 0.9) and args.fixed_input:  # execute only once
                            print(f'[TRAINING] High bit acc ({str_acc}) achieved, switch to full image dataset training.')
                            args.fixed_input = False
