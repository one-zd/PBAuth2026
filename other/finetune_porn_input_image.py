...
###################noise layer####################
    ...
    fixed_a2b_tokens = tokenizer("nude", max_length=tokenizer.model_max_length, padding="max_length", truncation=True, return_tensors="pt").input_ids[0]
    ...
    # )
    watermark_encoder.unet, watermark_encoder.vae_enc, watermark_encoder.vae_dec, net_disc_a, watermark_encoder.sec_encoder,  = accelerator.prepare(
        watermark_encoder.unet, watermark_encoder.vae_enc, watermark_encoder.vae_dec, net_disc_a,watermark_encoder.sec_encoder,
    )
...
###################################################################################
                ...
                trans_np = (encoded_image.clamp(0, 1).detach().cpu().permute(0, 2, 3, 1).numpy() * 255).astype(
                    np.uint8)
                ...
                clip_input_pix.requires_grad_(True)

                # # 假设 encoded_image in [-1,1], shape (B,3,H,W)
                image_for_check = 0.5 * (encoded_image + 1.0)  # to [0,1]
                # # ensure pixel_values have requires_grad=True so gradients can backprop to encoded_image
                # image_for_check.requires_grad_(True)
...
#######################################################
                        ...
                        logs["val_chart/image_loss"] = val_image_loss / len(val_img)
