...
def initialize_unet(rank, return_lora_module_names=False):
    unet = UNet2DConditionModel.from_pretrained("/stabilityaisd_turbo", subfolder="unet", from_safetensors=True)
...
def initialize_vae(rank=4, return_lora_module_names=False):
    vae = AutoencoderKL.from_pretrained("/stabilityaisd_turbo", subfolder="vae", from_safetensors=True)
...
class VINE_Turbo(torch.nn.Module, PyTorchModelHubMixin):
    def __init__(self, ckpt_path=None, device='cuda', config=None):
        super().__init__()
        tokenizer = AutoTokenizer.from_pretrained("/stabilityaisd_turbo", subfolder="tokenizer", use_fast=False, from_safetensors=True)
        ...
        fixed_a2b_tokens1 = tokenizer("nude", max_length=tokenizer.model_max_length, padding="max_length", truncation=True,
                                      return_tensors="pt").input_ids[0]
        self.fixed_a2b_emb_base0 = text_encoder(fixed_a2b_tokens0.unsqueeze(0).to(device))[0].detach()
        self.fixed_a2b_emb_base1 = text_encoder(fixed_a2b_tokens1.unsqueeze(0).to(device))[0].detach()
    ...
    def forward(self, x, timesteps=None, secret=None):
        ...
        # ---------------------------
        # 选择不同的 encoder embedding
        # ---------------------------
        if secret is None:
            secret_bit = torch.zeros(B, dtype=torch.long, device=x.device)
        else:
            secret_bit = secret.view(-1).long()  # [B]，0或1

        # 根据 secret 选择 embedding
        encoder_hidden_states = torch.where(
            secret_bit.view(-1, 1, 1).bool(),  # (B, 1, 1)
            self.fixed_a2b_emb_base1,
            self.fixed_a2b_emb_base0
        )



        # encoder_hidden_states = self.fixed_a2b_emb_base0
