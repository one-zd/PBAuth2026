...
from vine.src.Alias.vine_safety.stega_utils_input01 import StegaData, get_secret_acc, count_parameters
...
###################noise layer####################
    ...
    # params_sec = params_sec + list(decoder.parameters()) + list(watermark_encoder.sec_encoder.parameters())
    params_sec = params_sec + list(watermark_encoder.sec_encoder.parameters())
    
    params_gen = VINE_Turbo.get_traininable_params(unet=watermark_encoder.unet, vae_a2b=watermark_encoder.vae_a2b)
    # params_gen = params_gen + list(decoder.parameters()) + list(watermark_encoder.sec_encoder.parameters())
    ...
    # optimizer_disc = torch.optim.RMSprop(params_disc, lr=0.0001)     #####0.00001
    ...
    # )
    watermark_encoder.unet, watermark_encoder.vae_enc, watermark_encoder.vae_dec, watermark_encoder.sec_encoder,  = accelerator.prepare(
        watermark_encoder.unet, watermark_encoder.vae_enc, watermark_encoder.vae_dec, watermark_encoder.sec_encoder,
    )

    net_lpips, optimizer_gen, optimizer_sec, train_dataloader, lr_scheduler_sec, lr_scheduler_gen = accelerator.prepare(
        net_lpips, optimizer_gen, optimizer_sec, train_dataloader, lr_scheduler_sec, lr_scheduler_gen
    )
    ...
    for epoch in range(first_epoch, args.max_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            l_acc = [watermark_encoder.sec_encoder]
            with accelerator.accumulate(*l_acc):
                with accelerator.autocast():
                    ...
                    image_for_check = 0.5 * (encoded_image + 1.0)  # [B,3,H,W], 保持可微
    ...
    #####################################################################################
                    ...
                    ##### encoded_image = 2 * encoded_image - 1
                    # transformed_image = transform_net(encoded_image, img_a, global_step, args) # [-1, 1]
                    # transformed_image = transformed_image * 0.5 + 0.5 # [-1, 1] -> [0, 1]
                    ##### encoded_image = 0.5 * (encoded_image + 1)

                    # decoded_secret = decoder(transformed_image)
                    # decoded_secret = decoded_secret.to(dtype=weight_dtype)

                    # secret_label: from your dataset, shape (B,1) with values 0/1 (float tensor)
                    ...
                    # secret_loss = cross_entropy(has_nsfw, secret)
                    ...
                    # D_output_fake_forG, _ = net_disc_a(encoded_warped)
                    # G_loss = D_output_fake_forG.mean()
                    ...
                    if no_im_loss:
                        ...
                        # # 分步 backward，防止共用图
                        # accelerator.backward(loss_scales[0] * image_loss, retain_graph=True)
                        # accelerator.backward(loss_scales[1] * lpips_loss, retain_graph=True)
                        # accelerator.backward(loss_scales[2] * secret_loss, retain_graph=not args.no_gan)
                        # if not args.no_gan:
                        #     accelerator.backward(loss_scales[3] * G_loss)
                    ...
                    if global_step % args.validation_steps == 0:
                        ...
                        with torch.no_grad():
                            for i in tqdm(range(len(val_img)), desc="Validating", ncols=100):
                                ...
                                secret_val = np.ones(100, dtype=int)
...
##############################################################
                                ...
                                # 先把 transformed_image 转为 numpy array 列表供 feature_extractor 使用
                                trans_np = (encoded_image.clamp(0, 1).detach().cpu().permute(0, 2, 3,
                                                                                             1).numpy() * 255).astype(
                                    np.uint8)
                                clip_input = feature_extractor(images=list(trans_np), return_tensors="pt")
                                # 将 pixel_values 移到加速器设备
                                clip_input_pix = clip_input.pixel_values.to(accelerator.device)

                                # # 假设 encoded_image in [-1,1], shape (B,3,H,W)
                                image_for_check = 0.5 * (encoded_image + 1.0)  # to [0,1]


                                # call the private differentiable checker
                                out_images, nsfw_logits, nsfw_probs, has_nsfw = safety_checker(
                                    clip_input=clip_input_pix, images=image_for_check, blacken_images=False
                                )
