...
class DiffJPEGParams:
    def __init__(self):
        self.cache = {}

    def get(self, name, device, dtype, factory):
        key = (name, device, dtype)
        if key not in self.cache:
            self.cache[key] = factory(device, dtype)
        return self.cache[key]

_diff_jpeg_params = DiffJPEGParams()
...
def rgb_to_ycbcr_jpeg(image):
    def factory(device, dtype):
        matrix = torch.tensor(
            ...
            dtype=dtype).T
        shift = torch.tensor([0., 128., 128.], dtype=dtype) 
        return matrix.to(device), shift.to(device)

    matrix, shift = _diff_jpeg_params.get('rgb_to_ycbcr', image.device, image.dtype, factory)
...
def ycbcr_to_rgb_jpeg(image):
    ...
    matrix, shift = _diff_jpeg_params.get('ycbcr_to_rgb', image.device, image.dtype, factory)
...
def dct_8x8(image):
    def factory(device, dtype):
        ...
        tensor = torch.from_numpy(tensor).to(dtype=dtype, device=device)
        scale = torch.from_numpy(np.outer(alpha, alpha) * 0.25).to(dtype=dtype, device=device)
        return tensor, scale

    tensor, scale = _diff_jpeg_params.get('dct_8x8', image.device, image.dtype, factory)
...
def idct_8x8(image):
    def factory(device, dtype):
        alpha = np.array([1. / np.sqrt(2)] + [1] * 7)
        alpha = torch.from_numpy(np.outer(alpha, alpha)).to(dtype=dtype, device=device)
    ...
    tensor, alpha = _diff_jpeg_params.get('idct_8x8', image.device, image.dtype, factory)
...
def y_quantize(image, rounding, factor=1):
    ...
    def factory(device, dtype):
        y_table = torch.tensor(
            ...
            dtype=dtype).t().to(device)
        return y_table

    y_table = _diff_jpeg_params.get('y_table', image.device, image.dtype, factory)
...
def c_quantize(image, rounding, factor=1):
    # 5. Quantizaztion
    def factory(device, dtype):
        c_table = torch.empty((8, 8), dtype=dtype)
        c_table.fill_(99)
        c_table[:4, :4] = torch.tensor([[17, 18, 24, 47], [18, 21, 26, 66], 
                                        [24, 26, 56, 99], [47, 66, 99, 99]], 
                                        dtype=dtype).t()
        return c_table.to(device)

    c_table = _diff_jpeg_params.get('c_table', image.device, image.dtype, factory)
