...
from vine.src.Alias.training_src.transformations_strong import RewrittenTransformNet
...
from vine.src.Alias.freq_version.freq_loss_utils import get_freq_stats
...
####触发率训练accelerate launch --num_processes=1 --main_process_port 17736 vine/src/our/train_input01_o.py --enable_xformers_memory_efficient_attention --train_batch_size 2 --secret_loss_scale 10 --G_loss_scale 0.1 --l2_loss_scale 0.1 --lpips_loss_scale 0.1 --tracker_project_name finetuning_porn --key_change finetuning_porn --learning_rate 1e-4 --fixed_input
####图像质量训练accelerate launch --num_processes=1 --main_process_port 17736 vine/src/our/train_input01_o.py --enable_xformers_memory_efficient_attention --train_batch_size 2 --secret_loss_scale 5 --G_loss_scale 0.5 --l2_loss_scale 1.0 --lpips_loss_scale 0.5 --tracker_project_name train01 --key_change train01 --learning_rate 1e-4 --fixed_input
...
def main(args):
    ...
    # print("E:\phd//4\code\VINE//vine\src\output//finetuning_porn1112\checkpoint-100000")
...
###################noise layer####################
    ...
    #####################################################################################
                    ...
                    # 恢复原始GAN损失逻辑：Discriminator 训练目标是 Real < Fake
                    # Generator 训练目标是最小化 D(fake)，即让 fake 变小（接近 real）
                    ...
                    if no_im_loss:
                        ...
                        if not args.no_gan:
                            # 每10个step才训练一次判别器，给生成器更多机会
                            # if global_step % 10 == 0:
                                # # 为真图添加更大的噪声，增加判别难度
                                # noise_real = torch.randn_like(img_a) * 0.05
                                # real_noisy = 0.5 * (img_a + 1) + noise_real
                                # real_noisy = torch.clamp(real_noisy, 0, 1)
                                
                                # # 为水印图也添加更大的噪声
                                # noise_fake = torch.randn_like(encoded_warped) * 0.05  
                                # fake_noisy = encoded_warped + noise_fake
                                ...
                                # 恢复原始GAN损失逻辑：最小化 (Real - Fake) => Real 变小，Fake 变大
                                # 这样 D 会把 Real 推向负无穷，Fake 推向正无穷

                                accelerator.backward(D_loss, retain_graph=False)
                                if accelerator.sync_gradients:
                                    accelerator.clip_grad_norm_(list(net_disc_a.parameters()), 0.25)  # 更严格的梯度裁剪
                                optimizer_disc.step()
                    ...
                    if global_step % args.validation_steps == 0:
                        ...
                        with torch.no_grad():
                            for i in tqdm(range(len(val_img)), desc="Validating", ncols=100):
                                ...
                                if secret_flag == 0:
                                    secret_val = np.zeros(100, dtype=int)  # 全0数组
                                else:
                                    secret_val = np.ones(100, dtype=int)  # 全1数组
