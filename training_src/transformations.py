...
from vine.src.Alias.training_src.other_noises import Crop, Cropout, Dropout, Resize
...
class TransformNet(nn.Module):
    ...
    def forward(self, encoded_image, cover_img, global_step, args, p=0.999):
        ...
        if hasattr(self, 'imagenetc') and random.random() < 0.5 and global_step > args.imagenetc_step: 
        ...
        step0_val = self.step0.item()
        ramp_fn = lambda ramp: np.max([0., np.min([(global_step-step0_val) / ramp, 1.])])

        ### cropout/dropout and resize
        if random.random() < 0.5 and global_step > args.crop_resize_step:
            a = random.random()
            # if a <= 0.25:
            ...
            if global_step < 6 * self.up_level_interval + args.crop_resize_step:
                ...
                max_tamper_area = np.random.uniform(0.3, 0.5)
            ...
            encoded_image = self.cropout([encoded_image, cover_img], max_tamper_area=max_tamper_area, 
                                         height_ratio_range=(max(1 - max_tamper_area, 0.5), max(1 - max_tamper_area, 0.9)), 
                                         width_ratio_range=(max(1 - max_tamper_area, 0.5), max(1 - max_tamper_area, 0.9)))[0]
        ...
        ### ig filter
        if random.random() < 0.5 and global_step > args.ig_filter_step:
            encoded_image = self.ig_filter(encoded_image)
            encoded_image = encoded_image * 2 - 1  # [0, 1] -> [-1, 1]
            return encoded_image
        
        rnd_noise = torch.rand(1, device=encoded_image.device) * ramp_fn(args.rnd_noise_ramp) * args.rnd_noise
        ...
        rnd_sat = torch.rand(1, device=encoded_image.device) * ramp_fn(args.rnd_sat_ramp) * args.rnd_sat
        ...
        if not args.no_gaussianNoise:
            noise = torch.randn(encoded_image.size(), device=encoded_image.device, dtype=encoded_image.dtype) * rnd_noise
        ...
        if global_step < 10000:
            jpeg_quality = 100. - torch.rand(1, device=encoded_image.device) * ramp_fn(args.jpeg_quality_ramp) * (100. - args.jpeg_quality)
            jpeg_factor = torch.where(jpeg_quality < 50, 5000. / jpeg_quality, 200. - jpeg_quality * 2)
...
class ImagenetCTransform(nn.Module):
    ...
    def forward(self, x, corrupt_strength=None):
        ...
        # Forward: x + residual (where residual = distorted - original)
        # Backward: gradient flows through x, residual is treated as constant
...
class IG_Filter(nn.Module):
    ...
    def forward(self, x):
        ...
        for single_img in x:
            ...
            # Straight-Through Estimator:
            # Calculate residual using detached input to avoid cancelling gradients
            # residual = distorted - original
            residual = filtered_img_tensor - single_img.detach()
