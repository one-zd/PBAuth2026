...
class StegaData(Dataset):
    def __init__(self, data_path, secret_size=100, size=(512, 512)):
        ...
        # self.files_list += glob(os.path.join('E:\phd//4\code\VINE\dataset\openimage//valid', '*.jpg'))   #####test
        self.files_list += glob(os.path.join('E:\phd//4\code\VINE\dataset\openimage//valid', '*.jpg'))  ###validation
        # self.files_list += glob(os.path.join('E:\phd//4\code\VINE\dataset\openimage//test', '*.jpg'))    ####train
...
#############################################
        # 1. 随机生成一个0或1的标记（二选一，概率各50%）
        # np.random.randint(0, 2)：生成[0,2)区间的整数，即只能是0或1
...
def get_secret_acc(secret_true, secret_pred):
    ...
    secret_pred = torch.round(secret_pred)                    ########### 对预测结果进行四舍五入，将概率值转换为0或1的离散值
