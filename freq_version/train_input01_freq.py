...
from vine.src.Alias.freq_version.freq_loss_utils import get_freq_stats, loss_lowfreq
...
#####修改encoder为safe check，bit为0或1，不加编辑操作，先训练触发率#####合并sec和gen两个分支####
####$env:PYTHONPATH="E:\phd\4\code\VINE"
####set PYTHONPATH=E:\phd\4\code\VINE
####触发率训练accelerate launch --num_processes=1 --main_process_port 17736 vine/src/our/freq_version/train_input01_freq.py --enable_xformers_memory_efficient_attention --train_batch_size 2 --secret_loss_scale 10 --G_loss_scale 0.1 --l2_loss_scale 0.1 --lpips_loss_scale 0.1 --freq_loss_scale 0.5 --tracker_project_name finetuning_porn --key_change finetuning_porn --learning_rate 1e-4 --fixed_input
####图像质量训练set PYTHONPATH=E:\phd\4\code\VINE
####触发率训练accelerate launch --num_processes=1 --main_process_port 17736 vine/src/our/freq_version/train_input01_freq.py --enable_xformers_memory_efficient_attention --train_batch_size 2 --secret_loss_scale 1.5 --G_loss_scale 0.5 --l2_loss_scale 2.0 --lpips_loss_scale 1.5 --freq_loss_scale 0.1 --tracker_project_name finetuning_porn --key_change finetuning_porn --learning_rate 1e-4 --fixed_input
...
def main(args):
    ...
    watermark_encoder = VINE_Turbo("E:\phd//4\code\VINE//vine\src\output//finetuning_porn1130\checkpoint-200000")
...
###################noise layer####################
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            with accelerator.accumulate(*l_acc):
                with accelerator.autocast():
                    ...
                    if encoded_image.shape[-1] != clip_size_w or encoded_image.shape[-2] != clip_size_h:
    ...
    #####################################################################################
                    ...
                    if global_step > args.freq_constraint_start_step:
                        # 使用低频损失函数
                        freq_loss = loss_lowfreq(
                            residual=watermark_residual,
                            lf_radius=args.lf_radius,
                            mf_radius=args.mf_radius,
                            weight=0.4
                        )
                        freq_loss = freq_loss.to(accelerator.device)
                        freq_loss_dict = {'freq_total_loss': freq_loss.item()}
                    else:
                        freq_loss = torch.tensor(0.0, device=accelerator.device, requires_grad=True)
                        freq_loss_dict = {'freq_total_loss': 0.0}
                    #################
                    ...
                    #     if (global_step - switch_step) >= 700:
            ...
            logs["psnr"] = avg_psnr
            # if not args.no_gan:
            #     logs["ls_gan"] = G_loss.detach().item()
            #     if not no_im_loss:
            #         logs["ls_D"] = D_loss.detach().item()

            # [新增] 频域损失和统计
            # if global_step > args.freq_constraint_start_step:
            logs["freq_loss"] = freq_loss_dict['freq_total_loss']
                # logs["freq_energy_loss"] = freq_loss_dict['freq_energy_loss']
                # logs["freq_smooth_loss"] = freq_loss_dict['freq_smooth_loss']
